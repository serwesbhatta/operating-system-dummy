...
def Read_file(fsDB: SqliteCRUD, pid: int, filename: str, user_id: int):
    """
    ...
    """
    filters = {"name": filename, "pid": pid, "oid" : user_id}
    if fsDB:
        response = fsDB.get_file_content(filename, filters)
        if response["success"]:
